"""
Experiment script for the Bank Marketing dataset.

This script loads the direct marketing dataset from the UCI repository
(41,188 calls), derives a binary target `Y` (subscription) and a
sensitive attribute `S` based on age (1 if age ≥ 40, 0 otherwise).
It drops the `duration` column (to avoid leakage) and performs
one-hot encoding of categorical variables.  A logistic regression
model is trained and evaluated using baseline fairness metrics,
followed by marginal and group-conditional conformal prediction (CP)
with optional tuning of group-specific miscoverage levels.

Usage:
    python bank_marketing_experiment.py

Place `bank-full.csv` from the UCI Bank Marketing dataset in the
`data/` directory.  The CSV uses `;` as the delimiter.
"""
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    # Target: 1 if subscribed (y == 'yes'), 0 otherwise
    df["Y"] = (df["y"].str.lower() == "yes").astype(int)
    # Sensitive attribute: 1 if age ≥ 40, else 0
    df["S"] = (df["age"] >= 40).astype(int)
...
def oversample_training(X, y, S):
    """No oversampling for the Bank Marketing experiment."""
    return X, y, S
    ...
    for a0 in alpha_grid:
        for a1 in alpha_grid:
            ...
            if best_score is None or score < best_score:
                ...
                best_alphas = (a0, a1)
    # Print results
    print("Bank Marketing experiment")
